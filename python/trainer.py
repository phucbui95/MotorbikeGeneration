...
def train(args, dataset, generator, discriminator, g_optimizer, d_optimizer,
    ...
    for i in pbar:
        ...
        if used_sample > args.phase * 2:
            ...
            resolution = 4 * 2 ** step
            #TODO: change to reloading dataset
            loader = sample_data(
                dataset, args.batch.get(resolution, args.batch_default), resolution
            )
            data_loader = iter(loader)
...
if __name__ == '__main__':
    ...
    generator = nn.DataParallel(StyledGenerator(args.conv_dim, code_size))
    discriminator = nn.DataParallel(
        Discriminator(args.conv_dim, from_rgb_activate=not args.no_from_rgb_activate)
    )
    g_running = StyledGenerator(args.conv_dim, code_size)
    ...
    from dataset import MultipleResMotorbikeDS

    dataset = MultipleResMotorbikeDS(args.path)
    ...
    print("G", count_parameters(generator))
    print("D", count_parameters(discriminator))
